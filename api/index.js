...
app.get('/api/auth/seed', async (req, res) => {
  try {
    await connectDB();
    if (await User.countDocuments() > 0) return res.json({ message: 'Already seeded', seeded: false });
    await User.create([
...
app.get('/api/analytics/ranking', async (req, res) => {
  try {
    ...
    results.forEach(r => {
      ...
      if (!id) return;
      if (!map[id]) map[id] = { name: r.student.name, studentId: r.student.studentId, scores: [] };
...
app.get('/api/analytics/trends', async (req, res) => {
  try {
    ...
    const trends = {};
    results.forEach(r => {
      const s = r.session?.name || 'Unknown';
      if (!trends[s]) trends[s] = {};
      const subj = r.subject?.name || 'Unknown';
      if (!trends[s][subj]) trends[s][subj] = [];
      trends[s][subj].push(r.score);
    });
...
app.post('/api/results', async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      await existing.save();
      const p = await existing.populate(['student', 'subject', 'session']);
      return res.json({ result: p, message: 'Updated' });
    }
...
app.get('/api', (req, res) => {
  res.json({ status: 'ok', message: 'School Dashboard API' });
});
